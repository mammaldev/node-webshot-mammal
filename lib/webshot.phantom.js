...
var whenLoadFinished = function(status) {
  ...
  function renderCleanUpExit() {
    // get page details 
    console.log(options.capturePageDetails)
    if(options.capturePageDetails) {
        ...
        var metaTags =  Array.prototype.slice.call(document.getElementsByTagName("meta"))
          .map(function(item) {
            return {
              ...
              "metaProperty": item.getAttribute("property")
            };
          });
      ...
      var wf = fs.write(path.replace(/\.[a-zA-Z]*$/, ".json"), JSON.stringify(pageInfo));
    }
